...
<template>
  <div class="max-w-5xl mx-auto px-6 py-10">
    ...
    <!-- Back -->
    <NuxtLink to="/mentor/projekty" class="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-navy transition-colors mb-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
      <div>
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="project.program === 'Program A' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'">
            {{ project.program }}
          </span>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-lg border border-gray-100 p-6">
          <div class="flex items-center justify-between mb-4">
            ...
            <span class="text-xs text-gray-400">{{ completedMilestones }}/{{ project.milestones.length }} dokončených</span>
          </div>
          ...
          <div class="space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-3 mb-2">
                <div class="flex items-start gap-3">
                  ...
                  <div class="mt-0.5 shrink-0">
                    ...
                    <Clock v-else-if="milestone.status === 'pending_approval'" class="w-5 h-5 text-warning-500" />
              ...
              <!-- Description -->
              <p v-if="milestone.description" class="text-xs text-gray-500 ml-8 mb-2">{{ milestone.description }}</p>

              <!-- Comments thread -->
              <div v-if="milestone.comments.length" class="ml-8 space-y-2 mt-2">
                <div v-for="comment in milestone.comments" :key="comment.id" class="flex gap-2">
          ...
          <div class="space-y-4">
            ...
            >
              <div class="flex items-start justify-between gap-3 mb-2">
                ...
                <button @click="editConsultation(c)" class="text-gray-400 hover:text-blue-600 transition-colors">
              ...
              <div v-if="c.actionItems.length" class="space-y-1">
                ...
                <ul class="space-y-1">
                  <li v-for="item in c.actionItems" :key="item" class="flex items-start gap-2 text-xs text-gray-600">
      ...
      <div class="space-y-4">
        ...
        <div class="bg-white rounded-lg border border-gray-100 p-5">
          ...
          <div class="text-3xl font-bold text-navy mb-1">{{ Math.round((completedMilestones / project.milestones.length) * 100) }}%</div>
          <div class="bg-gray-100 rounded-full h-2 mb-2">
            <div class="h-2 rounded-full bg-purple-500" :style="{ width: `${(completedMilestones / project.milestones.length) * 100}%` }" />
          </div>
    ...
    <div v-if="showConsultationModal" class="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div class="relative bg-white rounded-xl shadow-lg p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between mb-5">
          ...
          <button @click="closeConsultationModal" class="text-gray-400 hover:text-gray-600"><X class="w-5 h-5" /></button>
        </div>
        ...
        <div class="space-y-4">
          ...
          />
          <FormField
            :field="{ name: 'summary', type: 'textarea', label: 'Záznam / zhrnutie', placeholder: 'Čo sa riešilo, aké závery padli, na čom tím pracuje...', required: true }"
          ...
          <div>
            ...
            <div class="space-y-2">
              <div v-for="(_, i) in consultationForm.actionItems" :key="i" class="flex gap-2">
                <input
                ...
                />
                <button type="button" @click="consultationForm.actionItems.splice(i, 1)" class="text-gray-400 hover:text-danger-500">
                  <X class="w-4 h-4" />
                </button>
              </div>
              <button type="button" @click="consultationForm.actionItems.push('')" class="inline-flex items-center gap-1.5 text-sm text-purple-600 hover:text-purple-800">
        ...
        <div v-if="consultationError" class="mt-4 bg-danger-50 border border-danger-200 text-danger-700 px-3 py-2 rounded text-sm flex items-center gap-2">
          <AlertCircle class="w-4 h-4 shrink-0" /> {{ consultationError }}
        </div>

        <div class="flex gap-3 mt-6">
          <button @click="closeConsultationModal" class="flex-1 px-4 py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50">
            Zrušiť
          </button>
          <button
...
import {
  ChevronLeft, Flag, MessageSquare, Plus, CheckCircle, Clock,
  Circle, X, Send, Pencil, ArrowRight, AlertCircle, Users
...
const project = reactive({
  ...
  milestones: [
    {
      id: 1, title: 'Analýza a návrh architektúry', dueDate: '20.03.2026',
      status: 'completed', description: 'Dokumentácia technickej architektúry, ERD diagram, API kontrakt.',
      comments: [
        { id: 1, author: 'Martin Kováč', date: '21.03.2026', text: 'Skvelá práca, architektúra je čistá. Odporúčam pridať rate limiting do API kontraktu.' },
        { id: 2, author: 'Tomáš Horváth', date: '21.03.2026', text: 'Zapracované, ďakujeme za feedback.' },
      ],
    },
    {
      id: 2, title: 'MVP — funkčný chatbot', dueDate: '30.04.2026',
      status: 'pending_approval', description: 'Základná funkcionalita chatbota s integráciou OpenAI API a jednoduchou webovou konzolou.',
      comments: [
        { id: 3, author: 'Tomáš Horváth', date: '01.04.2026', text: 'MVP je hotový, čakáme na schválenie mentora.' },
      ],
    },
    {
      id: 3, title: 'Integrácia so Salesforce CRM', dueDate: '31.05.2026',
      status: 'in_progress', description: 'Napojenie chatbota na zákaznícke dáta v Salesforce cez REST API.',
      comments: [],
    },
    {
      id: 4, title: 'Finálne odovzdanie', dueDate: '30.06.2026',
      status: 'pending', description: 'Produkčný deployment, dokumentácia, záverečná prezentácia.',
      comments: [],
    },
  ],
  consultations: [
    {
      id: 1, title: 'Kick-off konzultácia', date: '05.03.2026', duration: 90, type: 'Osobne',
      summary: 'Úvodné stretnutie s tímom. Predstavenie projektu, definícia cieľov, rozdelenie rolí. Dohodnutý spôsob komunikácie (Slack + týždenné sync).',
      actionItems: ['Vytvoriť repo a nastaviť CI/CD pipeline', 'Pripraviť technickú špecifikáciu do 10.03.'],
    },
    {
      id: 2, title: 'Review architektúry', date: '22.03.2026', duration: 60, type: 'Online (videohovor)',
      summary: 'Tím prezentoval návrh architektúry. Diskusia o voľbe LLM providera (OpenAI vs. self-hosted). Rozhodnutie: OpenAI API pre MVP, self-hosted pre produkciu.',
      actionItems: ['Pridať rate limiting do API dizajnu', 'Zdokumentovať fallback scenáre pre API výpadok'],
    },
    {
      id: 3, title: 'Sprint review #2', date: '01.04.2026', duration: 45, type: 'Online (videohovor)',
      summary: 'Tím predviedol funkčnú demo verziu chatbota. Kvalita odpovedí je dobrá. Chýba handling edge-case otázok mimo scope. Odoslaný na schválenie míľnika.',
      actionItems: ['Otestovať edge cases a pridať fallback odpovede', 'Pripraviť dokumentáciu pre Salesforce integráciu'],
    },
  ],
})
...
const closeConsultationModal = () => {
  ...
  Object.assign(consultationForm, { title: '', date: today, duration: 60, type: 'online', summary: '', actionItems: [''] })
}
...
const typeLabel = (type: string) => ({ online: 'Online (videohovor)', personal: 'Osobne', written: 'Písomná / e-mail' }[type] ?? type)
