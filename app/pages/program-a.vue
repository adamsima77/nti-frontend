    ...
    <template v-if="faq_pending || !faq">
      <div class="space-y-4">
        <div
          v-for="i in 5"
          :key="'faq-skeleton-' + i"
          class="bg-white border border-gray-200 rounded-lg p-6 flex items-center justify-between"
        >
...
const { faq, faq_pending  } = useFaq(PageType.PROGRAM_A)
...
const { banner, pending: banner_pending } = useBanner(PageType.PROGRAM_A)
const { t } = useI18n() 
