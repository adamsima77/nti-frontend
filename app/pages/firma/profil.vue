          ...
          <div class="flex flex-wrap gap-2">
            <label v-for="s in allSectors" :key="s.id" class="cursor-pointer">
              ...
              <span
                ...
                :class="form.sectors.includes(s.id) ? 'bg-blue-600 text-white border-blue-600 shadow-sm' : 'bg-white text-gray-500 border-gray-200 hover:border-blue-300 hover:text-blue-600'"
              >
                {{ s.name }}
              </span>
            </label>
...
const form = reactive({
  ...
  phone: '',
  sectors: [] as number[],
  address: { city: '', street: '', postalCode: '', country: '' },
})
const errors = reactive<Record<string, string | undefined>>({})

function setFormFromOrganization(data: any) {
...
const loadSectors = async () => {
  try {
    ...
    allSectors.value = response.map((s: any) => ({ id: s.id, name: s.sector_translations?.[0]?.name ?? s.name ?? '' }))
  } catch {}
}
...
const loadOrganization = async () => {
  ...
  if (!orgId) return
  const response = await api.get(`/organizations/${orgId}`) as any
  if (!response?.organization) throw new Error('Organizácia sa nenašla.')
...
const handleSave = async () => {
  ...
  try {
    const payload = {
      ...
      address: { city: form.address.city || null, street: form.address.street || null, postal_code: form.address.postalCode || null, country: form.address.country || null },
    }
    const response = await api.put(`/organizations/${orgId}`, payload) as any
    originalOrganization.value = response
    setFormFromOrganization(response)
