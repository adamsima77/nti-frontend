  ...
  <div class="min-h-screen flex flex-col">
    ...
    <div class="bg-gradient-to-br from-navy via-blue-900 to-blue-800 text-white py-20 px-6">
      <div class="max-w-7xl mx-auto">
        <h1 class="text-5xl font-bold mb-4">Výzvy a termíny</h1>
        <p class="text-xl text-blue-100">Pozrite si dostupné výzvy a zúčastnite sa tých, ktoré vás inšpirujú</p>
      </div>
    </div>
    ...
    <div class="flex-1 bg-gray-50 py-20 px-6">
      <div class="max-w-7xl mx-auto">
        ...
        <div v-if="callsStore.isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="i in 6" :key="i" class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 h-96 animate-pulse" />
        </div>

        <!-- Empty state -->
        <div v-else-if="callsStore.calls.length === 0" class="bg-white rounded-lg shadow-md border border-gray-200 p-12">
          <UiEmptyState
            :icon="Megaphone"
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          >
            <div class="bg-white rounded-lg shadow-md border border-gray-200 p-6 h-full hover:shadow-xl hover:border-blue-400 transition-all cursor-pointer flex flex-col">
              ...
              <!-- Title and program -->
              <h3 class="text-xl font-bold text-navy mb-2 group-hover:text-blue-600 transition line-clamp-2">{{ call.title }}</h3>
              <p class="text-sm text-gray-500 mb-5 font-medium">{{ call.programName }}</p>

              <!-- Description -->
              <p class="text-sm text-gray-600 mb-6 flex-grow line-clamp-3 leading-relaxed">{{ call.description }}</p>
              ...
              <div class="space-y-3 mb-6 pt-6 border-t border-gray-200">
                ...
                <div class="flex items-center gap-3 text-sm text-gray-700">
                  ...
                  <span>{{ call.applicantsCount || 0 }} {{ call.applicantsCount === 1 ? 'prihláška' : 'prihlášok' }}</span>
                </div>
              </div>

              <!-- CTA -->
              <button
                class="w-full px-6 py-3 text-sm font-semibold text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition mt-auto flex items-center justify-center gap-2 shadow-sm"
              >
                Zobraziť detail
