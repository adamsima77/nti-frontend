    ...
    <div class="w-full max-w-md">
      ...
      <div v-else-if="!invite" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center space-y-5">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500">Tento odkaz je neplatný alebo jeho platnosť vypršala. Kontaktujte administrátora NTI.</p>
        </div>
      </div>
      ...
      <div v-else-if="success" class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center space-y-5">
        ...
        <div>
          ...
          <p class="text-sm text-gray-500">
            ...
            Po schválení budete môcť sa prihlásiť ako hodnotiteľ.
          </p>
        </div>
        <NuxtLink
      ...
      <!-- Form -->
      <template v-else>
        <form class="bg-white rounded-xl shadow-sm border border-gray-200 p-8 space-y-5" @submit.prevent="handleAccept">
          ...
          <div class="text-center mb-2">
            ...
            <h1 class="text-2xl font-bold text-navy mb-1">Pozvánka do komisie</h1>
            <p class="text-sm text-gray-500">
              Boli ste pozvaní do hodnotiacej komisie
              <strong class="text-navy">{{ invite.commission_name }}</strong>
            </p>
          </div>
          ...
          <div class="grid grid-cols-2 gap-3">
            <UiInput v-model="form.name" type="text" label="Meno" placeholder="Ján" required :error="errors.name" />
            <UiInput v-model="form.surname" type="text" label="Priezvisko" placeholder="Novák" required :error="errors.surname" />
          </div>

          <!-- Password -->
          <UiInput v-model="form.password" type="password" label="Nové heslo" placeholder="••••••••••" required :error="errors.password" />
          ...
          <!-- Confirm password -->
          <UiInput v-model="form.password_confirmation" type="password" label="Potvrdiť heslo" placeholder="••••••••••" required :error="errors.password_confirmation" />
...
useHead({ title: 'Pozvánka do komisie | NTI' })
...
onMounted(async () => {
  ...
  try {
    const res = await api.get(`/auth/commission-invite?token=${encodeURIComponent(token.value)}`) as any
...
const strengthLabel    = computed(() => ['', 'Slabé', 'Primerané', 'Dobré', 'Silné'][strengthScore.value] ?? 'Slabé')
const strengthTextColor = computed(() => ['', 'text-red-500', 'text-yellow-500', 'text-blue-600', 'text-green-500'][strengthScore.value] ?? 'text-red-500')

const validate = () => {
  errors.name = errors.surname = errors.password = errors.password_confirmation = null
  let valid = true
  ...
  if (form.password.length < 8) { errors.password = 'Heslo musí mať aspoň 8 znakov'; valid = false }
  if (form.password !== form.password_confirmation) { errors.password_confirmation = 'Heslá sa nezhodujú'; valid = false }
  return valid
}
...
const handleAccept = async () => {
  ...
  try {
    await api.post('/auth/accept-commission-invite', {
      token:                 token.value,
      name:                  form.name,
